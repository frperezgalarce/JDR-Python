...
def run_self_tests():
    ...
    points = np.array([[0], [0.1], [4], [4.1], [9], [9.1]], float)
    D = squared_matrix(points)
    labels, info = fit_models(D, points, PROTOCOL["seed"])
    optimal = min(np.min(D[:, ids], axis=1).sum() for ids in combinations(range(6), 3))
    np.testing.assert_allclose(info["medoid_objective"], optimal, atol=1e-12)
    assert adjusted_rand_score(np.repeat(range(3), 2), labels["K-medoids"]) == 1
