...
def fit_models(D, z, seed):
    ...
    for start in range(PROTOCOL["n_init"]):
        medoids = np.random.default_rng(seed + start).choice(len(D), PROTOCOL["n_clusters"], replace=False)
        for iteration in range(300):
            labels = np.argmin(D[:, medoids], axis=1)
            labels[medoids] = np.arange(PROTOCOL["n_clusters"])
            updated = []
            for k in range(PROTOCOL["n_clusters"]):
    ...
    labels = {
        "K-medoids": best[1],
        "K-means": KMeans(n_clusters=PROTOCOL["n_clusters"], n_init=20, random_state=seed).fit_predict(z),
        "Average linkage": AgglomerativeClustering(
            n_clusters=PROTOCOL["n_clusters"], metric="precomputed", linkage="average"
        ).fit_predict(D),
...
def run_pipeline(data_dir, output):
    ...
    with threadpool_limits(limits=1):
        ...
        permutation = np.random.default_rng(PROTOCOL["seed"]).permutation(len(table))
        permuted = np.array(
        ...
        audit = []
        upper = np.triu_indices(len(table), 1)
        ...
        for replicate in range(PROTOCOL["subsamples"]):
            indices = np.sort(rng.choice(len(table), size=int(PROTOCOL["subset_fraction"]*len(table)), replace=False))
        ...
        summary = dict(
            ...
            interpretation="Exploratory in-sample representation comparison. Classical full explicitly retains period/amplitude; shape control removes those scalars. No catalog-based feature/hyperparameter selection. Scope is recorded in protocol.run_mode; pilot results are not full-catalog evidence.",
        )
        ...
        provenance = dict(
            ...
            acquisition_sha256=sha256(
                (Path(data_dir) / "sources_complete.json").read_bytes()
            ).hexdigest(),
            python=sys.version,
