...
CLASS_COLORS = {"RRab": "#0072B2", "RRc": "#D55E00", "RRd": "#009E73", "RRe": "#CC79A7"}
FIG_STYLE = {
...
def draw_figures(output):
    ...
    with plt.rc_context(FIG_STYLE):
        ...
        for i, (ax, representation) in enumerate(zip(axes, REPRESENTATIONS)):
            for kind, marker in zip(TYPES, ["o", "^", "D", "v"]):
        ...
        for i, representation in enumerate(REPRESENTATIONS):
            ...
            left = np.zeros(PROTOCOL["n_clusters"])
            for kind in TYPES:
                counts = (
    ...
    captions = {
        "figure_1_representation_comparison": "Figure 1 | Representation comparison with identical clustering policies on the selected original light curves. JDR is contrasted with 13 standardized classical shape features and those same features augmented by log period, log magnitude SD and log Q95−Q05 amplitude (16 total). ARI/AMI retain rejected points under label −1; coverage is shown separately. Stability intervals are empirical 10th–90th percentiles over 30 shared 80%-size subsets, with median markers. Imputation and scaling are refitted inside every classical-feature subset. These are stability summaries, not confidence intervals, and no model is selected from the catalog scores. All matrix methods receive squared dissimilarities in the primary comparison.",
        "figure_2_representation_geometry": "Figure 2 | Two-dimensional PCA views of the three representations. All selected objects are shown, colored by catalog subtype with distinct markers; pilot results cannot establish full-catalog performance. Each PCA is fitted separately; axes and explained-variance fractions refer to different spaces and are not directly comparable. Models use full representations, never these two coordinates. Dense point layers are rasterized at 600 dpi in vector exports.",
        "figure_3_cluster_composition_retention": "Figure 3 | Primary K-medoids composition and all-model retention. Top: counts in all four medoid clusters for each representation, with observed sizes; IDs are arbitrary across panels. Bottom: assigned fraction within each catalog subtype for every model, including all rejected stars in the denominator. All cluster compositions, including density-method noise, are exported in cluster_composition.csv. Fully assigned partitions need not recover astrophysical classes.",
